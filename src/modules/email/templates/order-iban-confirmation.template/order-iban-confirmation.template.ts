...
function formatDeliveryMethod(data: OrderIbanConfirmationData): string {
        ...
        <p style="font-size:15px;line-height:1.6;color:#555555;margin:0;">
          ${
				data.variant === 'payment_method_changed'
					? `Спосіб оплати замовлення ${data.orderNumber} змінено на банківський переказ (IBAN): після підтвердження вам прийде емейл з реквізитами на оплату.`
					: `Замовлення ${data.orderNumber} успішно створено, після підтвердження вам прийде емейл з реквізитами на оплату. Гарного дня!`
			}
        </p>
