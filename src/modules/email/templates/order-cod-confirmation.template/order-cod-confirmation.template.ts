...
export interface OrderCodConfirmationData {
	orderNumber: string
	/**
...
function formatDelivery(data: OrderCodConfirmationData): string {
...
function formatDeliveryMethod(data: OrderCodConfirmationData): string {
...
export function orderCodConfirmationTemplate(data: OrderCodConfirmationData): string {
        ...
        <p style="font-size:15px;line-height:1.6;color:#555555;margin:0;">
          ${
				data.variant === 'payment_method_changed'
					? `Спосіб оплати замовлення ${data.orderNumber} змінено на накладний платіж: оплата при отриманні у відділенні Нової Пошти.`
					: `Замовлення ${data.orderNumber} успішно створено. Оплата — накладним платежем при отриманні у відділенні Нової Пошти.`
			}
        </p>

        <div style="margin:20px 0 0;padding:16px 18px;background-color:#fffbeb;border:1px solid #fde68a;border-radius:12px;">
          <p style="margin:0 0 8px;font-size:14px;font-weight:700;color:#92400e;">Потрібна часткова передоплата</p>
          <p style="margin:0;font-size:14px;line-height:1.6;color:#78350f;">
            Відправка накладним платежем здійснюється за умови часткової передоплати — щонайменше 200 ₴.
            Наш менеджер зв'яжеться з вами найближчим часом і уточнить суму.
