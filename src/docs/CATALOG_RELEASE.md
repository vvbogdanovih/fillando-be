...
the same mount; and reports land in `scripts/migrations/reports/` **on the host** (gitignored).
The §0 slug-collision check runs the same way — `dist/` is inside the image. The `node
scripts/migrations/…` lines below are what goes after `api` in that invocation.

**Every script takes `--dry-run` and prints its full plan before it writes anything — always run
the dry run first and read it.** Each script is idempotent, so a re-run after a fix is safe.
