...
const CONCURRENCY = 10 // number of variants processed in parallel
const DELAY_MS = 150 // delay between HTTP requests per worker
...
async function main() {
	...
	/** Process one variant. Returns true if an HTTP request was made (worker should pause). */
	...
	async function worker() {
		while (nextIndex < variants.length) {
			const i = nextIndex++
			const madeRequest = await processVariant(i)
			if (madeRequest && nextIndex < variants.length) await sleep(DELAY_MS)
		}
	}
