...
| `liqpay_checkout_started_at` | Date \| null               | nullable — when the last LiqPay checkout payload was handed out; a second one within 15 minutes for a `PENDING` payment is refused (`LIQPAY_SESSION_ACTIVE`, TD-0009). A `FAILED` payment is retried at once, but the granted retry is stamped **and** put back to `PENDING`, so it opens the same 15-minute window: the second tab of a declined order is refused as well |
...
Two fields of the customer-facing reads (`POST /orders`, `GET /orders/me`, `GET /orders/me/:id`,
`PATCH /orders/me/:id/payment-method`) are **not** stored: `can_change_payment_method` and
`liqpay_retry_after_seconds` are derived per request — from `payment_status` / `order_status` and
from `liqpay_checkout_started_at` — by the same helpers the public lookup uses
(`src/modules/order/helpers/`), so the cabinet and the success page can never show different
clocks. `null` seconds means no card session was ever opened (pay at once), `0` means the window
is over. The storefront reads that number, never the stamp: `liqpay_checkout_started_at` is an
internal field and the 15-minute rule may only be evaluated server-side.
